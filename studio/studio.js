...
function buildProps(kind,B,amp,ov){
  ...
  switch(kind){
   ...
   case"orbit3d": {const R=a*1.6;p=SMP(t=>{const q=2*Math.PI*t;return [256+R*Math.sin(q),256+R*0.25*Math.cos(q),0];});s=SMP(t=>{const v=100+a*0.45*Math.cos(2*Math.PI*t);return [v,v,100];});break;}
   // --- Сюжетные ---
